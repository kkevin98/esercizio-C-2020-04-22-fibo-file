...
int main(int argc, char *argv[]) {
	...
	printf("Ora salvo i valori dentro un file:\n.\n.\n.\n");
	...
	int n_string=40*2-1;
	char * text_to_write[n_string];
	int f=0;

	//preparo il testo che andrò a scrivere all'interno del file

	for(int i=0; i<n_string; i++){
		if((i%2)==0){
			char * str;
			str=itoa(fibonacci_result[f]);
			text_to_write[i]=str;
			f++;
		}
		else{
	...
	for(int i=0; i<n_string; i++){
		res = write(fd, text_to_write[i], strlen(text_to_write[i]));
		if (res == -1) {
			perror("write()");
			exit(EXIT_FAILURE);
		}
	...
	printf("Ho scritto quello che volevi sul file con successo!!\n");
